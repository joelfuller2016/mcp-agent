...
class AdaptiveOrchestrator:
    ...
    async def initialize(self):
        ...
        if not self.available_servers:
            self.logger.warning(
                "No MCP servers discovered - some functionality may be limited"
            )
        else:
    ...
    async def execute_autonomous_task(
        self, task_description: str, preferences: Optional[Dict[str, Any]] = None
    ) -> ExecutionResult:
        """
        ...
        try:
            ...
            task_analysis, strategy_recommendation = (
                self.decision_engine.analyze_and_recommend(
            ...
            agents = self.agent_factory.create_agents_for_task(
                task_analysis,
                strategy_recommendation.pattern,
                list(self.available_servers.values()),
            )
    ...
    async def _ensure_tools_for_task(self, task_description: str):
        ...
        required_tools = await self.discovery_system.ensure_tools_for_task(
            task_description
        )
        ...
        async with self.app.run() as mcp_app:
            if pattern == WorkflowPattern.DIRECT:
                return await self._execute_direct(
                    task_description, agents[0], llm_factory
                )

            elif pattern == WorkflowPattern.PARALLEL:
                return await self._execute_parallel(
                    task_description, agents, llm_factory
                )
    ...
    async def _execute_direct(
        self, task_description: str, agent: Agent, llm_factory: Type[AugmentedLLM]
    ) -> str:
    ...
    async def _execute_router(
        ...
        router = LLMRouter(llm=llm_factory(), agents=agents)
    ...
    async def _execute_orchestrator(
        ...
        orchestrator = Orchestrator(
            llm_factory=llm_factory, available_agents=worker_agents
        )
    ...
    async def explain_approach(self, task_description: str) -> str:
        ...
        explanation = self.decision_engine.explain_decision(
            task_analysis, strategy_recommendation
        )
    ...
    def _show_status(self):
        ...
        print(
            f"  Server names: {', '.join(status['server_names'][:5])}{'...' if len(status['server_names']) > 5 else ''}"
        )
        print(f"  Tasks executed: {status['execution_history_count']}")
        print(
            f"  Success rate: {status['successful_executions']}/{status['execution_history_count']}"
        )
        print(f"  LLM provider: {status['llm_provider']}")
        print(
            f"  Available specializations: {len(status['specializations_available'])}\\n"
        )
...
async def run_demo():
    ...
    print(
        f"Executed {status['execution_history_count']} tasks with {status['successful_executions']} successes"
    )
