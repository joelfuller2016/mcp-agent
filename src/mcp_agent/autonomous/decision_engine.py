...
from .discovery import MCPServerProfile, ToolCapability
...
class TaskAnalyzer:
    ...
    def _load_complexity_keywords(self) -> Dict[TaskComplexity, List[str]]:
        ...
        return {
            ...
            TaskComplexity.EXPERT: [
                "orchestrate", "comprehensive analysis", "end-to-end", "full lifecycle",
                "strategic planning", "complex workflow", "enterprise-scale"
            ]
        }
    ...
    def _load_capability_keywords(self) -> Dict[str, List[str]]:
        ...
        return {
            "file_management": [
                "file", "folder", "directory", "document", "save", "load",
                "read file", "write file", "organize files"
            ],
            "web_automation": [
                "website", "web page", "browser", "navigate", "click", "form",
                "scrape", "screenshot", "web data"
            ],
            "information_retrieval": [
                "search", "find information", "look up", "research", "google",
                "web search", "query", "discover"
            ],
            "development": [
                "code", "repository", "github", "git", "programming", "software",
                "commit", "pull request", "issue", "development"
            ],
            "database": [
                "database", "query", "sql", "data analysis", "table", "records",
                "sqlite", "postgres", "data storage"
            ],
            "cognitive": [
                "think", "reason", "analyze", "decide", "plan", "strategy",
            ...
            "communication": [
                "message", "email", "chat", "notification", "send", "receive",
                "slack", "teams", "communicate"
            ]
        }
    ...
    def analyze_task(self, task_description: str) -> TaskAnalysis:
        ...
        self.logger.info(f"Task analysis complete: complexity={complexity.name}, "
    ...
    def _estimate_steps(self, task_description: str, complexity: TaskComplexity) -> int:
        ...
        step_indicators = task_description.lower().count(" and ") + \
    ...
    def _is_parallelizable(self, task_description: str) -> bool:
        ...
        sequential_indicators = [
            "then", "after", "next", "following", "subsequently", "once",
            "step by step", "in order", "sequential"
        ]
        
        task_lower = task_description.lower()
        
        parallel_score = sum(1 for indicator in parallel_indicators 
                           if indicator in task_lower)
        sequential_score = sum(1 for indicator in sequential_indicators 
                             if indicator in task_lower)
    ...
    def _requires_iteration(self, task_description: str) -> bool:
        ...
        iteration_indicators = [
            "improve", "optimize", "refine", "iterate", "perfect", "enhance",
            "better", "quality", "review", "feedback", "revise"
        ]
    ...
    def _requires_human_input(self, task_description: str) -> bool:
        ...
        human_indicators = [
            "approve", "confirm", "review", "check with", "ask", "verify",
            "permission", "authorization", "human", "manual", "interactive"
        ]
...
class StrategySelector:
    ...
    def select_strategy(self, task_analysis: TaskAnalysis, 
                       available_servers: List[MCPServerProfile]) -> StrategyRecommendation:
        """Select the optimal strategy for a task"""
        self.logger.debug(f"Selecting strategy for {task_analysis.complexity.name} task")
        
        # Score each pattern
        pattern_scores = {}
        for pattern in WorkflowPattern:
            pattern_scores[pattern] = self._score_pattern(pattern, task_analysis, available_servers)
        ...
        self.logger.info(f"Selected strategy: {best_pattern.value} "
                        f"(confidence: {best_score:.2f})")
        
        return recommendation
    
    def _score_pattern(self, pattern: WorkflowPattern, task_analysis: TaskAnalysis,
                      available_servers: List[MCPServerProfile]) -> float:
        ...
        if "multi_agent_coordination" in criteria and criteria["multi_agent_coordination"]:
            total_criteria += 1
            if (task_analysis.complexity.value >= TaskComplexity.ADVANCED.value and
                len(task_analysis.required_capabilities) > 2):
                score += 1.0
            else:
                score -= 0.5
    ...
    def _generate_reasoning(self, pattern: WorkflowPattern, 
        ...
        if pattern == WorkflowPattern.DIRECT:
            specific_reasons.append(f"Task complexity is {task_analysis.complexity.name}")
            specific_reasons.append(f"Only {task_analysis.estimated_steps} steps required")
        
        elif pattern == WorkflowPattern.PARALLEL:
            specific_reasons.append("Task can be parallelized")
            specific_reasons.append(f"Multiple capabilities needed: {task_analysis.required_capabilities}")
        
        elif pattern == WorkflowPattern.ORCHESTRATOR:
            specific_reasons.append(f"Complex task with {task_analysis.estimated_steps} steps")
    ...
    def _identify_required_servers(self, pattern: WorkflowPattern, 
                                 task_analysis: TaskAnalysis,
                                 available_servers: List[MCPServerProfile]) -> List[str]:
    ...
    def _estimate_execution_time(self, pattern: WorkflowPattern, 
                               task_analysis: TaskAnalysis) -> int:
    ...
    def _select_fallback_patterns(self, pattern_scores: Dict[WorkflowPattern, float],
                                best_pattern: WorkflowPattern) -> List[WorkflowPattern]:
...
class AutonomousDecisionEngine:
    ...
    def analyze_and_recommend(self, task_description: str,
                            available_servers: List[MCPServerProfile]) -> Tuple[TaskAnalysis, StrategyRecommendation]:
        ...
        self.logger.info(f"Decision complete: {strategy_recommendation.pattern.value} "
                        f"pattern recommended with {strategy_recommendation.confidence:.2f} confidence")
        
        return task_analysis, strategy_recommendation
    
    def explain_decision(self, task_analysis: TaskAnalysis, 
                        strategy_recommendation: StrategyRecommendation) -> str:
