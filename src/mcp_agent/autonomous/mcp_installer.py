...
class MCPInstaller:
    ...
    def __init__(self):
        ...
        self.capability_recommendations = {
            ToolCapability.FILE_OPERATIONS: [
                "filesystem", "google-drive", "dropbox", "onedrive"
            ],
            ToolCapability.WEB_SEARCH: [
                "brave-search", "google-search", "bing-search", "web-crawler"
            ],
            ToolCapability.DATABASE: [
                "sqlite", "postgres", "mysql", "mongodb"
            ],
            ToolCapability.AUTOMATION: [
                "puppeteer", "playwright", "selenium", "n8n"
            ],
            ToolCapability.DEVELOPMENT: [
                "github", "gitlab", "git", "docker", "kubernetes"
            ],
            ToolCapability.COMMUNICATION: [
                "slack", "discord", "teams", "email", "telegram"
            ],
            ToolCapability.ANALYSIS: [
                "pandas", "jupyter", "matplotlib", "plotly"
            ],
            ToolCapability.REASONING: [
                "sequential-thinking", "mcp-reasoner", "langchain"
            ],
            ToolCapability.WORKFLOW: [
                "temporal", "airflow", "n8n", "zapier"
            ],
            ToolCapability.SYSTEM: [
                "shell", "docker", "kubernetes", "aws-cli"
            ],
            ToolCapability.GRAPHICS: [
                "blender", "figma", "canva", "imagemagick"
            ],
    ...
    async def find_servers_for_capability(self, 
                                        capability: ToolCapability,
                                        max_results: int = 5) -> List[MCPServerCandidate]:
    ...
    async def find_servers_for_task(self, 
                                   task_description: str,
                                   required_capabilities: Set[ToolCapability]) -> List[MCPServerCandidate]:
        ...
        for capability in required_capabilities:
            capability_candidates = await self.find_servers_for_capability(capability, 3)
        ...
        scored_candidates = self._score_candidates_for_task(unique_candidates, task_description)
    ...
    async def install_best_servers_for_capability(self, 
                                                capability: ToolCapability,
                                                max_installs: int = 2) -> List[str]:
        """Install the best servers for a capability."""
        candidates = await self.find_servers_for_capability(capability, max_installs * 2)
    ...
    async def _create_candidate_from_name(self, 
                                        name: str, 
                                        capability: ToolCapability) -> Optional[MCPServerCandidate]:
    ...
    async def _search_online_registries(self, 
                                      capability: ToolCapability) -> List[MCPServerCandidate]:
    ...
    async def _search_task_specific_servers(self, task_description: str) -> List[MCPServerCandidate]:
        """Search for task-specific servers."""
    ...
    def _score_candidates(self, candidates: List[MCPServerCandidate],
                         capability: ToolCapability) -> List[MCPServerCandidate]:
    ...
    def _score_candidates_for_task(self, candidates: List[MCPServerCandidate],
                                  task_description: str) -> List[MCPServerCandidate]:
    ...
    async def suggest_servers_for_enhancement(self, 
                                            current_capabilities: Set[ToolCapability],
                                            max_suggestions: int = 5) -> List[MCPServerCandidate]:
