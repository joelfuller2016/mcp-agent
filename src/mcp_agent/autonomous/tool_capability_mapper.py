...
class ToolCapabilityMapper:
    ...
    async def discover_all_capabilities(self) -> Dict[str, MCPServerProfile]:
        ...
        for i, result in enumerate(results):
            ...
            if isinstance(result, Exception):
                self.logger.warning(
                    f"Failed to discover capabilities for {server_name}: {result}"
                )
            else:
        ...
        self.logger.info(
            f"Discovered capabilities for {len(self.server_profiles)} servers"
        )
    ...
    async def _discover_server_capabilities(
        ...
        try:
            async with gen_client(server_name) as client:
                ...
                self.logger.debug(
                    f"Discovered {len(capabilities)} capabilities for {server_name}"
                )
                return profile
    ...
    def _categorize_tool(self, tool_name: str, description: str) -> ToolCategory:
        ...
        if any(
            ...
            for keyword in ["git", "github", "code", "build", "deploy"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["web", "fetch", "url", "http", "browser"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["reason", "think", "analyze", "cognitive"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["workflow", "automate", "task", "schedule"]
        ):
        ...
        if any(
            keyword in name_lower
            for keyword in ["analyze", "evaluate", "assess", "report"]
        ):
            return ToolCategory.ANALYSIS

        # Creation tools
        if any(
            keyword in name_lower for keyword in ["create", "generate", "build", "make"]
        ):
    ...
    def _infer_server_description(self, server_name: str, tools: List[Any]) -> str:
        ...
        if server_name == "fetch":
            ...
            return (
                f"Database server with {tool_count} tools for data storage and querying"
            )
        else:
    ...
    def get_capability_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "available_servers": len(
                [p for p in self.server_profiles.values() if p.is_available]
            ),
            "total_capabilities": sum(
                len(p.capabilities) for p in self.server_profiles.values()
            ),
