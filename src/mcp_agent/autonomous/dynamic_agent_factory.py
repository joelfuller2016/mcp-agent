...
class DynamicAgentFactory:
    ...
    def create_agents_for_task(
        self, task_analysis: TaskAnalysis, max_agents: int = 5
    ) -> List[Agent]:
        """
    ...
    def create_specialized_agent(
        self, specialization_name: str, custom_instruction: Optional[str] = None
    ) -> Optional[Agent]:
        """
    ...
    def _select_agent_blueprints(
        self, task_analysis: TaskAnalysis, max_agents: int
    ) -> List[AgentBlueprint]:
    ...
    def _score_specialization(
        self, specialization: AgentSpecialization, task_analysis: TaskAnalysis
    ) -> float:
    ...
    def _create_blueprint(self, specialization: AgentSpecialization) -> AgentBlueprint:
        ...
        available_spec_servers = [
            server
            for server in specialization.server_names
    ...
    def _create_fallback_agent(self, task_analysis: TaskAnalysis) -> Agent:
        ...
        available_servers = [
            name
            for name, profile in self.tool_mapper.server_profiles.items()
        ...
        return Agent(
            name="general_agent", instruction=instruction, server_names=server_names
        )
    ...
    def get_specialization_info(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "effectiveness": len(spec_servers.intersection(available_servers))
            / len(spec_servers),
        }
    ...
    def create_custom_agent(
        ...
        return Agent(name=name, instruction=instruction, server_names=server_names)
    ...
    def get_factory_status(self) -> Dict[str, Any]:
        ...
        available_servers = sum(
            1
            for profile in self.tool_mapper.server_profiles.values()
            if profile.is_available
        )
