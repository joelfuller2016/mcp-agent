...
from .tool_discovery import ToolDiscoveryAgent, ToolCapability
...
class DynamicInstaller:
    ...
    def __init__(self, tool_discovery: ToolDiscoveryAgent):
    ...
    def _load_installation_database(self):
        ...
        self.installation_db = {
            ...
            "fetch": {
                ...
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-fetch"},
            ...
            "git": {
                ...
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-git"},
                "description": "Git repository operations and version control",
            },
            "github": {
                "package": "mcp-server-github",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-github"},
            ...
            "sqlite": {
                ...
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-sqlite"},
                "description": "SQLite database operations",
            },
            "postgres": {
                "package": "mcp-server-postgres",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-postgres"},
                "description": "PostgreSQL database operations",
            },
            "slack": {
                "package": "mcp-server-slack",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-slack"},
                "description": "Slack messaging and communication",
            },
            "gdrive": {
                "package": "mcp-server-gdrive",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-gdrive"},
                "description": "Google Drive integration",
            },
            "time": {
                "package": "mcp-server-time",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-time"},
                "description": "Time and scheduling operations",
            },
            "memory": {
                "package": "mcp-server-memory",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-memory"},
            ...
            "youtube": {
                ...
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-youtube"},
                "description": "YouTube video search and analysis",
            },
            "todo": {
                "package": "mcp-server-todo",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-todo"},
                "description": "Task management and productivity",
            },
            "everart": {
                "package": "mcp-server-everart",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-everart"},
            ...
            "obsidian": {
                ...
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-obsidian"},
                "description": "Obsidian note-taking integration",
            },
            "notion": {
                "package": "mcp-server-notion",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-notion"},
                "description": "Notion workspace integration",
            },
            "jira": {
                "package": "mcp-server-jira",
                "methods": [InstallationMethod.UVX],
                "commands": {InstallationMethod.UVX: "uvx install mcp-server-jira"},
                "description": "Jira issue tracking integration",
            },
        }

    async def ensure_tools_available(
        ...
        if not candidate_tools:
            self.logger.warning(
                f"No installable tools found for capabilities: {required_capabilities}"
            )
            return []
    ...
    def _find_installable_tools(self, required_capabilities: List[str]) -> List[str]:
        ...
        for tool_name, tool_info in self.installation_db.items():
            ...
            if tool_name in self.tool_discovery.capability_registry:
                tool_capabilities = self.tool_discovery.capability_registry[
                    tool_name
                ].capabilities
    ...
    async def install_tool(self, tool_name: str) -> bool:
        ...
        for method in tool_info["methods"]:
            if await self._check_installation_method_available(method):
                ...
                if result.success:
                    self.logger.info(
                        f"Successfully installed {tool_name} using {method.value}"
                    )
                    return True
                else:
                    self.logger.warning(
                        f"Failed to install {tool_name} using {method.value}: {result.error}"
                    )
    ...
    async def _check_installation_method_available(
        self, method: InstallationMethod
    ) -> bool:
    ...
    async def _install_with_method(
        self, tool_name: str, method: InstallationMethod, tool_info: Dict[str, Any]
    ) -> InstallationResult:
    ...
    async def suggest_additional_tools(
        ...
        current_tools = self.tool_discovery.get_tools_for_capabilities(
    ...
    def get_installation_summary(self) -> Dict[str, Any]:
        ...
        successful = [
            name for name, result in self.installation_cache.items() if result.success
        ]
        failed = [
            name
            for name, result in self.installation_cache.items()
            if not result.success
        ]

        total_time = sum(
            result.installation_time for result in self.installation_cache.values()
        )
        ...
        try:
            ...
            async with asyncio.timeout(10):
                ...
                if server_info and server_info.get("connected", False):
                    self.logger.info(
                        f"Verified {tool_name} is installed and functional"
                    )
                    return True
                else:
                    self.logger.warning(
                        f"Tool {tool_name} installed but not functional"
                    )
                    return False
    ...
    async def uninstall_tool(self, tool_name: str) -> bool:
        ...
        try:
            ...
            if result.method == InstallationMethod.UVX:
                uninstall_cmd = (
                    f"uvx uninstall {self.installation_db[tool_name]['package']}"
                )
            elif result.method == InstallationMethod.NPM:
                uninstall_cmd = (
                    f"npm uninstall -g {self.installation_db[tool_name]['package']}"
                )
            elif result.method == InstallationMethod.PIP:
                uninstall_cmd = (
                    f"pip uninstall -y {self.installation_db[tool_name]['package']}"
                )
            else:
                self.logger.warning(
                    f"Uninstallation not supported for method {result.method.value}"
                )
                return False
