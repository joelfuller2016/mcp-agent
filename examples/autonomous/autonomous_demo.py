...
from mcp_agent.autonomous.autonomous_orchestrator import AutonomousOrchestrator, AutonomousConfig
...
async def demo_capability_analysis(orchestrator: AutonomousOrchestrator):
    ...
    print(f"  Available Servers: {tool_caps['available_servers']}/{tool_caps['total_servers']}")
...
async def demo_specialized_workflows():
    ...
    test_tasks = [
        ...
        ("Web research", "Search for recent news about Model Context Protocol and summarize findings"),
        ("Code analysis", "Analyze the Python code structure in the src directory"),
        ("Complex planning", "Create a development roadmap for adding voice capabilities to mcp-agent"),
        ("Data processing", "If there are any CSV files, analyze their structure and content")
    ]
    ...
    for task_name, task_description in test_tasks:
        ...
        print(f"  🎯 Suggested Pattern: {suggestions['strategy']['recommended_pattern']}")
        print(f"  🤖 Agents Needed: {suggestions['agents']['suggested_count']}")
        print(f"  🔧 Required Servers: {', '.join(suggestions['strategy']['required_servers'])}")
...
async def main():
    ...
    try:
        ...
        for i, execution in enumerate(history[-5:], 1):
            print(f"{i}. {execution['task'][:50]}... - {execution['pattern']} - {'✅' if execution['success'] else '❌'}")
