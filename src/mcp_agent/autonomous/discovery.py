...
from .capability_mapper import CapabilityMapper, ToolCapability
...
class AutonomousDiscovery:
        ...
        if hasattr(self.config, 'mcp') and hasattr(self.config.mcp, 'servers'):
            for server_name, server_config in self.config.mcp.servers.items():
                try:
                    ...
                    self.logger.warning(f"Error analyzing configured server {server_name}: {e}")
    ...
    async def _discover_from_common_locations(self) -> Dict[str, MCPToolInfo]:
        ...
        for server_name in common_servers:
            try:
                ...
                capabilities = await self.capability_mapper.get_default_capabilities(server_name)
    ...
    async def get_tools_for_capability(self, capability: str) -> List[MCPToolInfo]:
        ...
        return [self.discovered_tools[name] for name in tool_names 
                if name in self.discovered_tools]
    
    async def recommend_tools_for_task(self, task_description: str) -> List[MCPToolInfo]:
        """
    ...
    async def update_performance_metrics(self, tool_name: str, 
                                       response_time: float, 
                                       success: bool):
        ...
        metrics['reliability'] = min(1.0, metrics['success_rate'] + 
                                   (0.1 if response_time < 2.0 else -0.1))
