...
def test_import(module_name, class_name=None):
    ...
    try:
        ...
        print(f"[FAIL] {module_name}{('.' + class_name) if class_name else ''}: IMPORT ERROR - {e}")
        return False
    except AttributeError as e:
        print(f"[FAIL] {module_name}.{class_name}: ATTRIBUTE ERROR - {e}")
        return False
    except Exception as e:
        print(f"[FAIL] {module_name}{('.' + class_name) if class_name else ''}: UNEXPECTED ERROR - {e}")
...
def main():
    ...
    print(f"Core framework success rate: {core_success}/{len(core_imports)} ({core_success/len(core_imports)*100:.1f}%)")
    ...
    print(f"Autonomous module success rate: {autonomous_success}/{len(autonomous_imports)} ({autonomous_success/len(autonomous_imports)*100:.1f}%)")
    ...
    print(f"Capabilities module success rate: {capabilities_success}/{len(capabilities_imports)} ({capabilities_success/len(capabilities_imports)*100:.1f}%)")
    
    # Summary and recommendations
    print("\nDIAGNOSTIC SUMMARY")
    print("=" * 30)
    
    total_tests = len(core_imports) + len(autonomous_imports) + len(capabilities_imports)
