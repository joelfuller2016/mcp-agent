...
from .tool_capability_mapper import ToolCapabilityMapper, ToolCategory
...
class AutonomousStrategySelector:
    ...
    def __init__(self, discovery: AutonomousDiscovery):
        ...
        self.quality_strategy_preferences = {
            ...
            "high": [
                ExecutionStrategy.ORCHESTRATOR,
                ExecutionStrategy.EVALUATOR_OPTIMIZER,
            ],
            "critical": [
                ExecutionStrategy.EVALUATOR_OPTIMIZER,
                ExecutionStrategy.ADAPTIVE_HYBRID,
            ],
        }
    ...
    async def analyze_task(
        ...
        self.logger.info(
            f"Task analysis complete. Complexity: {complexity_score}, "
    ...
    async def recommend_strategy(
        ...
        sorted_strategies = sorted(
            strategy_scores.keys(), key=lambda s: strategy_scores[s], reverse=True
        )
        ...
        self.logger.info(
            f"Strategy recommendation: {best_strategy.value} "
    ...
    async def _calculate_complexity_score(
        self, task_description: str, capabilities: List[str]
    ) -> int:
        ...
        if any(
            ...
            for indicator in ["then", "after", "first", "next", "finally"]
        ):
            complexity += 2

        # Quality indicators
        if any(
            qual in task_lower for qual in ["perfect", "best", "optimal", "excellent"]
        ):
            complexity += 1

        return min(10, max(1, complexity))

    async def _detect_interdependencies(
        self, task_description: str, capabilities: List[str]
    ) -> bool:
    ...
    async def _assess_parallel_potential(
        self, task_description: str, capabilities: List[str]
    ) -> float:
    ...
    def _extract_quality_requirements(
    ...
    async def _assess_coordination_needs(
        self, task_description: str, capabilities: List[str]
    ) -> bool:
        ...
        return len(capabilities) > 3 or any(
            indicator in task_lower for indicator in coordination_indicators
        )
    ...
    async def _score_strategy(
        ...
        if strategy == ExecutionStrategy.DIRECT:
            ...
            score += (
                0.2
                if len(
                    set(cap.split(":")[0] for cap in task_analysis.tool_requirements)
                )
                > 1
                else 0.1
            )
            ...
            score += (
                0.4
                if task_analysis.quality_requirements in ["high", "critical"]
                else 0.1
            )
        ...
        if strategy.value in self.strategy_performance:
            historical_score = self.strategy_performance[strategy.value].get(
                "success_rate", 0.5
            )
        ...
        tool_availability = len(required_tools) / max(
            1, len(task_analysis.tool_requirements)
        )
    ...
    async def _generate_strategy_reasoning(
        ...
        reasons.append(
            f"Task complexity ({task_analysis.complexity_score}/10) fits {strategy.value} pattern"
        )
    ...
    async def _find_required_tools(
    ...
    def _estimate_duration(
        self, strategy: ExecutionStrategy, task_analysis: TaskAnalysis
    ) -> float:
        """Estimate execution duration in minutes"""

        base_duration = (
            task_analysis.complexity_score * 2
        )  # 2 minutes per complexity point
    ...
    def _estimate_success_probability(
        ...
        strategy_adjustments = {
            ...
            if task_analysis.complexity_score <= 3
            else -0.1,
            ExecutionStrategy.PARALLEL: 0.1
            if task_analysis.parallel_potential > 0.7
            else -0.1,
            ExecutionStrategy.ORCHESTRATOR: 0.1
            if task_analysis.coordination_needs
            else -0.1,
            ExecutionStrategy.EVALUATOR_OPTIMIZER: 0.2
            if task_analysis.quality_requirements == "critical"
            else 0.0,
        }
        ...
        if strategy.value in self.strategy_performance:
            historical_success = self.strategy_performance[strategy.value].get(
                "success_rate", 0.7
            )
    ...
    async def update_strategy_performance(
        ...
        self.logger.info(
            f"Updated performance for {strategy_key}: "
