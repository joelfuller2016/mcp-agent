...
class MetaCoordinator:
    ...
    async def execute_autonomous_task(
        ...
        try:
            ...
            self.logger.info(
                f"Task analysis complete: {task_analysis.complexity.value} - {task_analysis.suggested_pattern.value}"
            )
            ...
            self.logger.info(
                f"Selected strategy: {strategy.pattern.value} (confidence: {strategy.confidence_score:.2f})"
            )
    ...
    async def _ensure_tools_available(self, task_analysis: TaskAnalysis) -> List[str]:
        ...
        if suggested_tools:
            self.logger.info(
                f"Suggested tools for manual installation: {suggested_tools}"
            )
    ...
    async def _execute_with_pattern(self, context: ExecutionContext) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(
                ...
                agents_created=len(
                    context.strategy_recommendation.agent_configuration.get(
                        "agents", []
                    )
                ),
    ...
    async def _execute_orchestrator(self, context: ExecutionContext) -> str:
        ...
        orchestrator = Orchestrator(
            llm_factory=self.default_llm_factory, available_agents=available_agents
        )
    ...
    def _calculate_performance_metrics(
        ...
        return {
            ...
            "estimated_vs_actual_time": {
                "estimated": context.strategy_recommendation.estimated_resources.get(
                    "estimated_duration_minutes", 0
                )
                * 60,
                "actual": execution_time,
                "ratio": execution_time
                / max(
                    context.strategy_recommendation.estimated_resources.get(
    ...
    def _update_performance_metrics(self, result: ExecutionResult):
        ...
        for tool in result.tools_used:
            self.performance_metrics["tool_usage"][tool] = (
                self.performance_metrics["tool_usage"].get(tool, 0) + 1
            )

        # Track errors
        if not result.success and result.error_message:
            error_type = (
                type(result.error_message).__name__
                if hasattr(result.error_message, "__name__")
                else "General"
            )
            self.performance_metrics["error_patterns"][error_type] = (
                self.performance_metrics["error_patterns"].get(error_type, 0) + 1
            )
    ...
    async def analyze_capability_gaps(self, failed_tasks: List[str]) -> Dict[str, Any]:
        ...
        for task in failed_tasks:
            ...
            if not available_tools:
                gap_analysis["missing_capabilities"].update(
                    task_analysis.required_capabilities
                )
